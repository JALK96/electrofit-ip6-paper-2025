...
positive_ion.name           = "NA"
positive_ion.desired_count  = 12
positive_ion.concentration  = 0.150  # mol/L for that species

negative_ion.name           = "CL"
negative_ion.desired_count  = 12
...
1. **Config schema & compatibility**
   ...
   - Update the loader/parser to convert old configurations (`concentration`) into
     the new structure and emit a warning.
...
5. **GROMACS adapter changes**
   - Refactor `set_up_production` so geometry/ion logic lives near the top:
       ...
       * If in salt mode → behave as today, using `editconf -d`.
       ...
       * After solvation, call `genion` with explicit `-np/-nn` when counts are
         fixed; rely on `-conc` only in salt mode. When neutrality forces a computed
         counter-ion count, pass that via `-np`/`-nn` rather than letting `-neutral`
         adjust the fixed species.
   ...
   - Add logging to record: requested vs inferred counts, computed volume, resulting
     ...
     neutrality constraints are disabled or violated.
6. **Pipeline touch points**
   - `pipeline/steps/step3.py` and `domain/final_sim.py` currently extract
     ...
     neutrality flag) instead of individual floats.
...
7. **Documentation & migrations**
   ...
   - Provide a warning in logs when legacy keys are read or when neutrality is
     disabled.
...
1. **Schema update**
   - Modify `SimulationIonsSection` in `config/loader.py` to include
     ...
     dataclasses for `IonTarget` (name, desired_count, optional concentration flag,
     maybe charge sign).
...
4. **Adapter refactor**
   - Update `set_up_production` signature to accept the new ion spec; keep backward
     compatibility path for `conc` until all callers are migrated.
   - Insert geometry calculation before calling `editconf`; log computed box
     dimensions.
   - In target mode: call `genion` with `-np`/`-nn` derived from the helper. When
     neutrality is disabled, skip `-neutral` to prevent `genion` from altering counts.
5. **Call site changes**
...
7. **Testing strategy**
   ...
   - Tests ensuring warnings/errors trigger correctly when neutrality requirements
     are unmet or disabled, and when dual concentrations are specified.
